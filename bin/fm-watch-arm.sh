...
# <suffix> is appended verbatim (the recovery-generation handoff token, or empty).
report_started() {  # [suffix]
  local age
  age=$(fm_path_age "$BEAT")
  case "$age" in
    ''|*[!0-9]*) return 1 ;;
  ...
  echo "watcher: started pid=$child (beacon fresh)${1:-}"
}
...
while :; do
  if healthy_watcher; then
    if [ "$HEALTHY_PID" = "$child" ]; then
      ...
      if ! report_started "${handling_generation:+ recovery-generation=$handling_generation}"; then
        sleep 0.2
        continue
      fi
      wait "$child"
