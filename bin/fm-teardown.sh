...
# Every Herdr close and its presence gate already ran before the branch delete
# and worktree return above (teardown_herdr_close_and_confirm).
if [ "$BACKEND" != orca ] && [ "$BACKEND" != herdr ] && [ "$TEARDOWN_WINDOWLESS" != 1 ]; then
  fm_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "fm-$ID" \
