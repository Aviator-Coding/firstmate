...
\`resolved\` separately closes an escalated decision or blocker, and only a \`resolved [key=<slug>]:\` line carrying that decision's exact key closes it: a later \`done\` or \`working\` event never does, even when the answer is what started that work.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   A decision or blocker you opened stays open until a \`resolved [key=<slug>]:\` line carrying that same key lands; a later \`done:\` or \`working:\` line never closes it, even when the answer is what started that work.
