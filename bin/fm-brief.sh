...
**HARD SAFETY GATE:** this scaffold cannot inspect the task text filled into the `# Task` section below later.
...
Confirm with \`gh-axi\` that the PR exists for your pushed branch, then append \`done: PR {https:// url}\` to the status file and stop.
...
When it is implemented and committed, append \`done: ready in branch fm/$ID, not pushed\` to the status file and stop.
...
Then append \`ready: {summary}\` to the status file and stop - not \`done:\`, which this mode reserves for the PR.
...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), confirm with \`gh-axi\` that the PR exists for your pushed branch, append \`done: PR {https:// url} checks green\`, and stop. You are finished.
A \`done:\` without that PR URL is read as not delivered.
EOF
    ;;
esac
