...
# Crews append only wake-worthy transitions (done/ready/needs-decision/blocked/paused/failed)
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  # bin/fm-watch.sh's active_run_log_fresh reads this run id (through
  # crew_absorb_class). A CLI without the run-overview table leaves
  # SELECTED_RUN_ID empty, but a full-source working run is still this branch's
  # own run, so its id comes from the same status output.
  if [ -n "$SELECTED_RUN_ID" ]; then
    RUN_DETAIL="$RUN_DETAIL${SEP}run: $SELECTED_RUN_ID"
  elif [ "$RUN_STATE" = working ] && [ "$RUN_SOURCE" = full ]; then
    fallback_run_id=$(strip_quotes "$(nm_field id)")
    [ -z "$fallback_run_id" ] || RUN_DETAIL="$RUN_DETAIL${SEP}run: $fallback_run_id"
  fi
