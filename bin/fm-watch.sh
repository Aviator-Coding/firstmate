...
# The optional <active-run-file> (written by write_active_run at
# classification time) is consulted right after the wait consult: a pane
# whose active no-mistakes run is still appending to its own step log resets
# the idle timer instead of escalating (active_run_log_fresh) - a cheap stat,
# not a no-mistakes call. busy_turn_bound_check passes none, so a busy pane
# never consults it.
wedge_timer_check() {  # <window> <since-file> <triage-label> <escalation-count-file> <task> <pane-hash> [<active-run-file>]
  local win=$1 since_file=$2 label=$3 escalation_file=$4 task=$5 hash=$6 active_run_file=${7:-} since age n reason evidence
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            ...
            wedge_timer_check "$w" "$ssf" "stale (overridden terminal status)" "$ewf" "$task" "$h" "$arf"
          fi
            ...
            if [ -e "$pf" ] || status_is_paused_or_captain_held "$(last_status_line "$STATE/$task.status")"; then
              ...
              case "$pause_class_out" in
                ...
                working*) clear_pause_state "$key"
                         printf '%s' "$h" > "$sf"
                         write_active_run "$arf" "${pause_class_out#working }"
                         wedge_timer_check "$w" "$ssf" "non-terminal stale (provably working after a declared pause)" "$ewf" "$task" "$h" "$arf"
                         triage_log "absorbed non-terminal stale (provably working): $w" ;;
                *)       handle_paused_stale "$w" "$task" "$h" ;;
              esac
            else
              wedge_timer_check "$w" "$ssf" "non-terminal stale" "$ewf" "$task" "$h" "$arf"
            fi
          fi
        fi
      else
