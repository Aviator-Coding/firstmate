...
A matching PID and identity from a row written after that arm began following the cycle lets it report the delivered reason and exit zero even after its durable wake was handled and acknowledged.
An unrelated queue producer, a recycled PID, or a leftover row from an earlier cycle cannot satisfy the match.
Only a cycle with no matching new delivery record emits `watcher: FAILED - cycle ended without an actionable reason` and exits nonzero.
...
Only the watcher process touches `state/.last-watcher-beat`; it writes its own pid on every beat so a leftover fresh mtime cannot make a new or phantom holder look healthy.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy, a leftover delivery row that must not close a silent cycle, and a launch that never beats and must not be reported as started or attached.
