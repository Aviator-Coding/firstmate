...
A matching PID and identity from a row written after that arm began following the cycle lets it report the delivered reason and exit zero even after the durable wake queue was drained.
...
`tests/fm-watch-arm.test.sh` covers attached delivery after a drain, a leftover delivery row that must not close a silent cycle, and a launch that never beats and must not be reported as started or attached.
