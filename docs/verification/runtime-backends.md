...
The same teardown fixture was re-run on 2026-09-22, on the merge of upstream `884d76b` into the fork, after moving every refusable Herdr close and the post-close presence gate ahead of branch delete and treehouse return.
Cross-task slot exclusivity is upstream's slot-ownership proof and owner claim, pinned by `tests/fm-teardown-endpoint-safety.test.sh` and `tests/fm-secondmate-safety.test.sh`.
...
Observed guarantees: an active-tab projected close and an unknown post-close presence both refuse while the isolated copy, the task branch, and `worktree=` remain intact and treehouse is not invoked.
The refusable close also runs before the backlog close-marker is written, so a refused close leaves no `state/<id>.backlog-close` for bootstrap to replay against the retained record.
