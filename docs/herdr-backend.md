...
The mover connects to an AF_UNIX-safe spelling of that socket: the herdr-reported path is kept when physically resolving its parent (a home-manager or Nix-store symlink under `~/.config/herdr`) would exceed the platform `sun_path` limit.
...
Task cleanup acquires that session lock, then closes the exact pane and confirms it gone, before the task's isolated copy is returned to the pool: returning a pooled copy cannot be undone, while a contended lock, a refused active-tab close, and an unconfirmed close are all retryable, so each of those refusals leaves the copy, its task branch, every durable record, and the endpoint intact for a plain rerun.
...
It provisions only non-default names beginning with `fm-lab-`, scopes allowed task commands with a leading `--session` that no caller argument can displace, clears the ambient socket path so a flag failure still resolves to the lab, refuses caller-supplied session flags and server/session lifecycle subcommands, and performs destructive stop/delete only through its guarded lifecycle actions.
