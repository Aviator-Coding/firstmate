...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits and open-PR merge waits that remain idle past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
A provably-working stale pane backed by an active no-mistakes run whose own step log is still being written resets that timer instead of escalating, so a worker idling only because it polls the pipeline is never mistaken for a wedge; [configuration.md](configuration.md#stale-pane-wedge-escalation-threshold-configstale-escalate-secs--fm_stale_escalate_secs) owns the threshold and freshness-check contract.
...
For an ordinary crew that has stopped, the normal-mode watcher applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint from the first sighting when the backend confidently reports its agent dead.
A live or inconclusively read agent may still be sitting at a real decision gate, so it surfaces one stale wake first and only then joins the cadence.
That single surface is spent once per declared wait and tracked durably, never per pane hash, so an idle pane that merely repaints cannot re-spend it and flood the supervisor.
An idle ship crew whose PR has an armed, validated merge poll from `bin/fm-pr-check.sh` is waiting on that merge, so it takes the same pause cadence with no surface owed, even while its no-mistakes run keeps monitoring the PR and whether or not it also declared `paused:`.
That wait is keyed on the validated poll registration, never a bare `pr=` line, and holds only while the crew reads working or done with no later open decision, blocker, or failure and its agent is confidently alive; a disarmed or invalid poll, a parked or failed run, or a dead or unreadable endpoint returns to ordinary surfacing or wedge escalation (`pr_merge_wait_holds` in `bin/fm-watch.sh`).
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes the newest no-mistakes run on the crew's branch, keeps a current-code-matched run-step authoritative even if the pane has closed, and reports `branch_sync` rather than unknown or a superseded terminal outcome when a current run's head is not in the crew worktree.
The script header owns the exact run-selection and run-head ancestry rules.
...
A trailing `working:` line after that idle verdict is not current state: the helper reports unknown with the status text and the idle liveness verdict so a stopped agent cannot hide behind a phase note.
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies that same live process is named by a fresh liveness beacon, and prints an honest `started`, `attached`, or nonzero `FAILED` status.
...
Because a returned pool worktree can be handed to another task immediately, teardown also refuses before any mutation when another task's record still claims the same worktree, and `--force` does not override that refusal.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, and the shared-claim release procedure for a record whose worktree was already returned.
