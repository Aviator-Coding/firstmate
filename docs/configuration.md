...
A comma-joined `blocked-by: a,b` is not read back by tasks-axi at all: it is absorbed into the task title and the row reports no dependencies, so `tasks-axi ready` offers it for dispatch even though Bearings reads both ids as unresolved blockers.
...
That check runs after the declared-wait consult and before the worktree-write and dead-record probes, so a declared wait keeps its bounded recheck and the cheap log stat precedes the costlier probes.
...
The Kimi installer requires an existing regular non-symlink `~/.kimi-code/config.toml`, `python3`, and `jq`; it validates TOML with stdlib `tomllib` when present and otherwise with the bundled `bin/fm_toml` reader, never serializes the captain's TOML, and refuses before writing when the config is missing, malformed, or surprising or when either tool requirement is unavailable.
...
FM_BUSY_DEFER_MAX_SECS=900         # max seconds a busy verdict alone may withhold an away-mode escalation; past it inject proceeds while the composer guard stays absolute (bin/fm-supervise-daemon.sh header)
