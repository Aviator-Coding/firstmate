...
EOF
  ...
  assert_contains "$out" "state: unknown" "idle + trailing working: is not current state"
  assert_contains "$out" "stage 2 in progress" "the rewritten worktree's own log stays the reported source"
  assert_contains "$out" "harness idle (claude-hook)" "the idle liveness verdict is named"
...
test_uninitialized_idle_worker_uses_status() {
  ...
  out=$(run_crew_state "$d" worker)
  assert_contains "$out" 'state: unknown' 'R2 idle + trailing working: is not current state'
...
test_historical_inventory_uses_current_status() {
  ...
  out=$(run_crew_state "$d" competing)
  assert_contains "$out" 'state: unknown' 'R3 idle + trailing working: is not current state'
...
test_captured_completed_history() {
  ...
  for activity in busy idle; do
    ...
    want='state: working'; [ "$activity" = busy ] || want='state: unknown'
    out=$(run_crew_state "$d" competing)
    assert_contains "$out" "$want" 'captured completion does not hide subsequent development'
...
# A parked run record with NO top-level head must not bind through the nested
# branch_sync.local.head, which equals the worktree HEAD by construction.
test_missing_run_head_not_bound_via_branch_sync_local_head() {
  reset_fakes
  local d out wthead
  d=$(new_case missing-head-nested-local-head)
  make_repo_on_branch "$d/wt" fm/feat-bs-head
  wthead=$(git -C "$d/wt" rev-parse HEAD)
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/bs-head.meta" "window=fm:fm-bs-head" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: current stage still in progress\n' > "$d/state/bs-head.status"
  FM_FAKE_AXI_STATUS="$(run_parked fm/feat-bs-head | grep -v '^  head:')
branch_sync:
  state: synced
  local:
    branch: fm/feat-bs-head
    head: \"$wthead\"
    clean: true"
  FM_FAKE_RUNS_LIST=""
  FM_FAKE_BUSY=0
  arm_idle_record "$d/state" bs-head
  out=$(run_crew_state "$d" bs-head)
  assert_not_contains "$out" "source: run-step" "a missing run head must not bind via branch_sync.local.head"
  assert_not_contains "$out" "parked at" "an unverified parked record must not mask current state"
