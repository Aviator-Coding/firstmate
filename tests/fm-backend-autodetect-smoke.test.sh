...
# bin/fm-herdr-lab.sh, which scopes the call to the named session and verifies
# the default fleet session is unchanged after teardown. Never replace the
# helper with an ambient HERDR_SESSION-only command.
set -u
