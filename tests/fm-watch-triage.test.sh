...
test_classifier_primitives() {
  ...
  # Key positions a worker may state: before the colon, or at the head of the
  # note right after it. Both fold as that slug. A line with no token still
  # folds as default, and an unkeyed resolved: still closes default only.
  printf '%s\n' \
    ...
    > "$state/keys-positions.status"
  open=$(status_open_decisions "$state/keys-positions.status")
  ...
  printf '%s' "$open" | grep -F $'after\t' >/dev/null \
    || fail "a key token at the head of the note folded as default instead of the slug"
  ...
  [ -z "$(status_open_decisions "$state/loose-close.status")" ] \
    || fail "a resolved line with the key at the head of the note did not close that key"
  # A malformed token deep in an opener's prose is just prose: the escalation
  # still surfaces under default rather than being swallowed.
  printf '%s\n' \
  ...
  printf '%s' "$open" | grep -F $'default\tneeds-decision\t' >/dev/null \
    || fail "a needs-decision with a malformed key token in its prose was dropped instead of surfacing under default"
...
EOF
  ...
  # A key named only in an activity note must not open a phase: the phase's own
  # unkeyed terminal line could never close it, and an open phase is read as
