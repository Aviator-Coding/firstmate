...
# The drain prints the folded ledger key, including default, so the key offered
# to --resolve-key is the one the ledger holds. A [key=...] token in note prose
# folds under default and stays visible as note text beside [key=default].
test_open_decisions_print_the_folded_key() {
  local dir state out
  dir=$(make_case folded-key-display)
  state="$dir/state"
  out="$dir/drain.out"
  ...
  grep -F 'after [key=after] needs-decision: pick REST or RPC' "$out" >/dev/null \
    || fail "a note-head key was not printed as the folded key: $(cat "$out")"
  grep -F 'endline [key=default] needs-decision: review gate 3 ask-user findings [key=review-ask-user]' "$out" >/dev/null \
    || fail "a prose key token was not shown under its folded default key: $(cat "$out")"
  ...
  pass "OPEN DECISIONS prints the folded key, including default"
}
