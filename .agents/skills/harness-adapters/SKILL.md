...
## grok (VERIFIED 2026-06-29, grok 0.2.73; slash-submit re-verified 2026-07-03 on 0.2.82; reasoning-effort ceiling re-verified 2026-07-13 on 0.2.99; exit paths re-verified 2026-07-19 on grok 0.2.103; busy state and interrupt re-verified 2026-08-15 on grok 1.0.4 (d846eb93d94d) [stable])
...
| Fact | Value |
|---|---|
| Busy state | The one remaining rendered-tail fallback, isolated to Grok until its structured lifecycle is live-verified: the mid-turn cancel hint shown in grok's keybind bar iff a turn is running. Grok 0.2.x showed `Ctrl+c:cancel`; grok 1.0.x (verified 1.0.4) renamed it to `Esc:cancel` and both are matched. The idle bar shows neither hint, only `Shift+Tab:mode │ Ctrl+x:shortcuts` on 1.0.4 (`Ctrl+.:shortcuts` on 0.2.x). ASCII is matched rather than the braille spinner to avoid locale fragility. |
...
| Interrupt | `Ctrl+C` cancels the current turn on every verified version. On grok 1.0.4 `Esc` ALSO cancels the current turn (footer shows `Esc:cancel` mid-turn); on grok 0.2.x `Esc` only moved focus to the scrollback and did NOT interrupt - re-verify before relying on `Esc` against an 0.2.x pane. |
