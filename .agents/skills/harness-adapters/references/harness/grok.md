...
Verified initially on 2026-06-29 with 0.2.73, slash submission on 2026-07-03 with 0.2.82, effort on 2026-07-13 with 0.2.99, exit on 2026-07-19 with 0.2.103, and busy state and interrupt on 2026-08-15 with 1.0.4 (d846eb93d94d) [stable].
...
| Fact | Value |
|---|---|
| Busy state | The last rendered-tail fallback, isolated to Grok pending a semantic source: the ASCII mid-turn cancel hint, `Ctrl+c:cancel` on 0.2.x and `Esc:cancel` on 1.0.x (verified 1.0.4), both matched; the idle bar shows neither (`Shift+Tab:mode │ Ctrl+x:shortcuts` on 1.0.4, `Ctrl+.:shortcuts` on 0.2.x); never the locale-fragile braille spinner. |
...
| Interrupt | Single `Ctrl+C` on every verified version; on 1.0.4 Escape also cancels the turn (footer `Esc:cancel`), while on 0.2.x it only focused scrollback, so re-verify before relying on Escape against a 0.2.x pane. |
