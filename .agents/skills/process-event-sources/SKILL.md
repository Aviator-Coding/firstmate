...
For a plain local blocking command with no adapter of its own, whose every result firstmate reads and judges itself (not a deterministic condition->action pair):
...
`bin/fm-procevent.sh --help`, `bin/fm-procevent-lavish.sh --help`, `bin/fm-procevent-when.sh --help`, `bin/fm-procevent-quota.sh --help`, `bin/fm-procevent-remote-reply.sh --help`, and `bin/fm-procevent-longpoll.sh --help` own the exact commands and flags.
