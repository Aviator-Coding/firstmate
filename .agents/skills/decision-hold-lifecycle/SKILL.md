...
description: >-
  ...
  Load before treating an investigation, scout report, structured review, or Lavish review as complete, before ending a visual review that exposed a decision, when recording or routing the captain's answer, and when withdrawing a hold registered in error.
...
Dependent work that has already finished still resolves, so a decision the captain answered and the fleet delivered inside one session closes instead of sitting open and making the captain's decision list misleading.
A hold that should never have been asked, such as a duplicate registration or a choice already taken and executed elsewhere, closes through `bin/fm-decision-hold.sh withdraw` with a written reason instead.
Withdrawal is a distinct recorded outcome from a captain decision and is never a shortcut for one, so use it only when there is no answer to record.
...
7. Put the captain's exact durable decision in a file and use the script's `resolve` command with every routed task, including routed work that has already finished.
   When the hold should never have been asked, use the script's `withdraw` command with a written reason instead.
