...
Use only these prefixes for status changes: working:, needs-decision:, blocked:, paused:, ready:, done:, failed:.
...
Use ready: instead of done: for a no-mistakes-mode ship task's implementation-commit handoff; its brief's Definition of done says which verb closes the task.
