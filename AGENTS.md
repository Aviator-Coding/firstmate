...
config/stale-escalate-secs  optional durable override for how long an idle-but-provably-working pane may sit before the watcher escalates it as a possible wedge; LOCAL, gitignored; one positive integer, absent means the built-in default (240s); see docs/configuration.md "Stale-pane wedge escalation threshold"
...
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .active-run-* .dead-reported-* .writing-* .waiting-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
  .watch-triage.log  watcher's absorbed-wake debug log (size-capped); never relied on, safe to delete
  .last-watcher-beat watcher liveness beacon, rewritten with the beating pid every poll (including while absorbing benign wakes); guard scripts read its age and pid
...
For a no-mistakes ship, trigger validation on the same worker after its `ready:` implementation-commit handoff, using the harness invocation owned by `harness-adapters`.
